...
This is a deliberate constraint, not a v1 placeholder. Because we use LLM-driven extraction (see ADR-0005's PDF parsing context), silent extraction errors — dropped rows, swapped digits, sign flips — are a persistent risk. The reconciliation gate is the only mechanism that converts those silent errors into loud failures. Allowing partial commit, "best-effort" ingest, or warnings would defeat the gate: bad data would land in the dashboard exactly the way the gate exists to prevent.

## Consequences

- A failed Statement requires manual intervention (re-trigger, prompt fix, or hand-rolled parser). This is acceptable.
